...
#pragma omp target teams distribute parallel for if(N >= 1024) map(to: data[0:N]) map(tofrom: out[0:N])
    for (int i = 0; i < N; ++i) {
        float val = data[i];
...
#pragma omp target teams distribute parallel for if(rows >= 512) map(to: input[0:rows]) map(tofrom: output[0:rows])
